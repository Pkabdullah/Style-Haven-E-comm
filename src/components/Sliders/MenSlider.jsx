    ...
    async function getBanner() {
      try {
        let response = await fetch(
          `http://localhost:1337/api/men-sliders?populate=*`,
          {
            method: "GET",
            headers: {
        ...
        {menbanners.map((banner) => (
          <div key={banner.attributes.Title} className="relative h-full w-full">
            <Image
              ...
              className="object-cover w-[280px] h-[400px] rounded-xl border-4 border-transparent transition-transform transform hover:scale-105"
            />
